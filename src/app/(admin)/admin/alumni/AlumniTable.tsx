...
export default function AlumniTable({ initialResponse }: { initialResponse: any }) {
  ...
  const fetchData = useCallback(async (p: number, s: string) => {
    setLoading(true)
    const res = await getAlumniList(p, 10, s)
    if (res.success) {
      setData(res.data || [])
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-zinc-50 dark:bg-zinc-800/50 text-zinc-500 dark:text-zinc-400 text-xs uppercase tracking-wider">
                ...
                <th className="p-4 font-medium">{t("reg_date")}</th>
                ...
                <tr key={user.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                  ...
                  <td className="p-4">
                   <span className={`px-3 py-1 rounded-full text-xs font-medium border ${
                       user.profile?.status === 'APPROVED' ? "bg-green-50 text-green-600 border-green-200" :
                       user.profile?.status === 'WAITING' ? "bg-amber-50 text-amber-600 border-amber-200" :
